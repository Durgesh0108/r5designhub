...
export default function AdvertisementFormPage() {
	...
	const [selectedAdPrice, setSelectedAdPrice] = useState<string>("");
	...
	useEffect(() => {
		const fetchAdPrice = async () => {
			if (
				...
				setSelectedAdPrice(price[0]?.price);
				setAdPrice(price);
				// } else {
				//   const sizes = await getAdSize();
				//   console.log("all subdata", sizes);
